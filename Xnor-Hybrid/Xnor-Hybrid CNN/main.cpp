...
double jiemi_to_CLIENT(struct hybird A[],int n)
{
    double result=0.0;
    for(int i=0;i<n;i++)
    {
        if(A[i].decide==0)
        {
            result=result+(A[i].value)*pow(2,i);
        }
        if(A[i].decide==1)
        {
            int ai=bootsSymDecrypt(A[i].ciphertext,key);
            result=result+ai*pow(2,i);
        }
    }
    result=result/pow(2,15);
    return result;
}
...
void Fullconnection_Relu(struct HYBIRD_ARRAY_25 result[10],struct HYBIRD_ARRAY_25 input[10], struct HYBIRD_ARRAY_25 b_weight[10])
{
    for(int i=0;i<10;i++)
    {
        struct hybird T[25];
        full_adder_comcode(T,input[i].alone,b_weight[i].alone,25);
        copy_array(result[i].alone,T,25);
        delete_array(25,T);
        if((result[i].alone[24].decide==0)&(result[i].alone[24].value==1))
        {
            constant_array(result[i].alone,25);
        }
        if(result[i].alone[24].decide==1)
        {
            struct hybird ALICE;
            struct hybird TEMP[25];
            copy_hybird(ALICE,result[i].alone[24]);
            not_hybird(ALICE);
            for(int j=0;j<25;j++)
            {
                and_hybird(TEMP[j],ALICE,result[i].alone[j]);
            }
            copy_array(result[i].alone,TEMP,25);
            delete_array(25,TEMP);
            delete_hybird(ALICE);
        }
    }
...
void FINAL_FULLCONNECTION(struct HYBIRD_ARRAY_25 FINAL_RESULT[10],struct HYBIRD_ARRAY_25 input[432],int full_connection[4320])
{
    for(int m=0;m<10;m++)
    {
        ...
        finish=clock();
        totaltime=(double)(finish-start)/CLOCKS_PER_SEC;
        cout<<"full connection times:"<<m<<" "<<totaltime<<"秒！"<<endl;

    }
}
int main()
{
    cout<<"add time calculator"<<endl;
   ...
   finish=clock();
    ...
    ifstream infile12;
        ...
        clock_t start_full,finish_full;
        double totaltime_full;
        start_full=clock();

        FINAL_FULLCONNECTION(NEW_WORLD,INPUT_fullconnection,r_full);

        finish_full=clock();
        totaltime_full=(double)(finish_full-start_full)/CLOCKS_PER_SEC;
        cout<<"all of full_connection times:"<<totaltime_full<<"秒！"<<endl;

        float b_FC[10]={0.0530,4.5933,0.4191,-0.4588,-0.6371,1.5588,-1.1861,1.8038,-3.5537,-0.9022};
        int b_params[10];
        for(int i=0;i<10;i++)
        {
            double tmp=b_FC[i]*pow(2,15);
            b_params[i]=tmp;
        }
        struct HYBIRD_ARRAY_25 B_weight[10];
        for(int i=0;i<10;i++)
        {
            for(int j=0;j<25;j++)
            {
                B_weight[i].alone[j].decide=0;
                B_weight[i].alone[j].value=((b_params[i]>>j)&1);
            }
        }
        struct HYBIRD_ARRAY_25 client_result[10];
        Fullconnection_Relu(client_result,NEW_WORLD,B_weight);
        double record[10];
    for(int m=0;m<10;m++)
    {
        double tmps=jiemi_to_CLIENT(client_result[m].alone,25);
        record[m]=tmps;
        cout<<"number "<<m<<" : "<<tmps<<endl;
    }
    fstream outfile3;
    ...
    cout<<"CNN model times:"<<totaltime0<<"秒！"<<endl;
